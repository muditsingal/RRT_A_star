...
def rand_node_2():
	while True:
		rand_x = random.uniform(0, 420)
		rand_y = random.uniform(0, 200)
		if not check_obs(rand_x,rand_y):
			return rand_x, rand_y
...
def check_obs(x, y):
	obs_locs = np.array([[127.5, 45], [127.5, 100], [127.5, 155], [195, 7.5], [195, 67.5], [195, 132.5], [195, 192.5], [270, 45], [270, 100], [270, 155]])
	upper_locs = obs_locs + clearance + bot_radius + 7.5
	lower_locs = obs_locs - clearance - bot_radius - 7.5
	# Checking out of bounds
	if x <= (clearance + bot_radius) or x >= (420-(clearance + bot_radius)) or y <= (clearance + bot_radius) or (y >= 200-(clearance + bot_radius)):
		return 1

	pose_arr = np.array([x, y])
	res = np.logical_and(pose_arr >= lower_locs, pose_arr <= upper_locs)
...
def sample_k_pts(coords, k):
	...
	j = 0
	while j < k:
		rand_x, rand_y = rand_node() # Modify this
...
# bot_radius = 10.5
# ############### obstacle space ########
# x, y = np.meshgrid(np.arange(0, 420), np.arange(0, 200))
# obs_locs = np.array([[127.5, 45], [127.5, 100], [127.5, 155], [195, 7.5], [195, 67.5], [195, 132.5], [195, 192.5], [270, 45], [270, 100], [270, 155]])
# block_dim = 15.0/2
# c_r = clearance + bot_radius

# for obs in obs_locs:
# 	rect = (x>=obs[0]-block_dim-c_r) & (x<=obs[0]+block_dim + c_r) & (y>=obs[1] -block_dim - c_r) & (y<=obs[1] +block_dim + c_r)
# 	ax.fill(x[rect], y[rect], color='lightblue')
